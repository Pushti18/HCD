...
//   }
  ...
  if (move_uploaded_file($_FILES["photo"]["tmp_name"], $target_file)) {
      ...
      exit;
        ...
        $internship = "INSERT INTO technical (enroll,studentName,name_of_award,date,year,team,level,name_of_agency,contact_details,remarks,photo_of_award,receiving_award) 
        VALUES ('$user_id','$studentName','$Name_of_award','$Date','$year','$Team','$level','$Name_of_Awarding_agency','$contact_details','$remarks','$image_path', '$image_path2')";
        echo $internship;
        // exit(0);
        if (mysqli_query($conn, $internship)) {
            

            $id = "SELECT id FROM technical WHERE enroll = '$user_id'  ";
            $id_res = $conn->query($id);

            $row = mysqli_fetch_array($id_res,MYSQLI_ASSOC);

            
            header ("Location: demoadmin_technical.php");
