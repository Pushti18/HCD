...
if (isset($_POST["update"])) {
    ...
    if (isset($_FILES["receiving_photo"]) && !empty($_FILES["receiving_photo"]["tmp_name"])) {
        ...
        if ($imageFileType2 != "jpg" && $imageFileType2 != "jpeg" && $imageFileType2 != "png") {
            ...
            window.location.href = 'extension_update.php';
        </script>";
        ...
        $internship = "UPDATE extension set `name_of_award`='$Name_of_award', `date`='$Date', `year`='$year', `team`='$Team', `level`='$level', `name_of_agency`='$Name_of_Awarding_agency', `contact_details`='$contact_details', `photo_of_award`='$image_path', `receiving_award`='$image_path2' where `enroll`='$user_id' ";
        // echo $internship;
        // exit(0);
        if (mysqli_query($conn, $internship)) {
            $id = "SELECT id FROM extension WHERE enroll = '$user_id''";
