...
if (isset($_FILES["receiving_photo"]) && !empty($_FILES["receiving_photo"]["tmp_name"])) {
    ...
    if ($imageFileType2 != "jpg" && $imageFileType2 != "jpeg" && $imageFileType2 != "png") {
        ...
        window.location.href = 'technical_update.php';
    </script>";
    ...
    if (isset($_FILES["photo"]) && !empty($_FILES["photo"]["tmp_name"])) {
        ...
        $internship = "UPDATE technical set `name_of_award`='$Name_of_award', `date`='$Date', `year`='$year', `team`='$Team', `level`='$level', `name_of_agency`='$Name_of_Awarding_agency', `contact_details`='$contact_details',`remarks`='$remarks', `photo_of_award`='$image_path', `receiving_award`='$image_path2' where `enroll`='$user_id' ";
        // echo $internship;
        // exit(0);
